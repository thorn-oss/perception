...
def build_reference_df(filepaths: typing.Iterable[str],
  ...
  features = [
      generate_image_descriptors(
          filepath, max_features=max_features, min_features=min_features, max_size=max_size)
  ...
  return pd.DataFrame({
      ...
      'descriptor_count': [f[1].shape[0] if f is not None else None for f in features
                          ],  # type: ignore
...
def validate_match(kp1: np.ndarray,
  ...
  good_A2B, good_B2A = map(lambda distances: (distances[:, 0] < distances[:, 1] * ratio),
  ...
  ptsAt = cv2.perspectiveTransform(ptsA.reshape((-1, 1, 2)), MAB).reshape(-1, 2).clip(0, dimsB)
  ptsBt = cv2.perspectiveTransform(ptsB.reshape((-1, 1, 2)), MBA).reshape(-1, 2).clip(0, dimsA)
...
def deduplicate_sift_dfs(match_df: pd.DataFrame,
    ...
    for start in tqdm.tqdm(range(0, len(candidates), batch_size)):
      futures = {
          executor.submit(
              ...
              ratio=ratio): (c1, c2) for c1, c2 in candidates[start:start + batch_size]
      }
...
def deduplicate(
    ...
    query_filepaths_or_df: typing.Optional[typing.Union[typing.Iterable[str], pd.DataFrame]] = None,
