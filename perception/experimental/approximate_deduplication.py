...
ClusterAssignment = typing_extensions.TypedDict('ClusterAssignment', {'cluster': int, 'id': str})
...
def compute_euclidean_pairwise_duplicates_approx(X,
  ...
  """Provides the same result as perception.extensions.compute_pairwise_duplicates_simple
    ...
    """
  assert counts.sum() == X.shape[0], "Length of counts incompatible with vectors shape."
  assert (Y is None) == (y_counts is None), "Must provide both or neither for y, y_counts."
  if X.dtype != 'float32':
  ...
  if faiss_cache_path is not None and op.exists(faiss_cache_path):
    ...
    index = ad.build_index(X=X, pct_probe=pct_probe, approximate=True, use_gpu=use_gpu)
  ...
  for end, length, query in zip(iterator_counts.cumsum(), iterator_counts,
    ...
    matched = [
        match for match in np.unique(lookup[list(set(idxs))])  # type: ignore
...
def pairs_to_clusters(
    ...
    strictness: typing_extensions.Literal["clique", "community", "component"] = "clique",
  ...
  for component in nx.connected_components(graph):
    ...
    if strictness == "component":
      ...
      cluster_index += 1
      continue
    for community in nx.algorithms.community.asyn_lpa_communities(graph.subgraph(component)):
      LOGGER.debug("Got community with size: %s", len(community))
      if strictness == "community":
        assignments.extend([{"id": n, "cluster": cluster_index} for n in community])
        cluster_index += 1
        continue
      ...
      for start in range(0, len(community), max_clique_batch_size):
        ...
        while subgraph:
          ...
          assignments.extend([{"id": n, "cluster": cluster_index} for n in clique])
          cluster_index += 1
