...
def test_tmk_parity():
    ...
    for filepath in [
        ...
        features_timestamps = [(hasher.frame_hasher.compute(
            frame, hash_format='vector'), timestamp)
    ...
    for normalization in ['feat', 'feat_freq', 'matrix']:
        ...
        scores_theirs = model.tmk.merge(
            ...
            offsets=torch.from_numpy(offsets[np.newaxis, ]))[0]
